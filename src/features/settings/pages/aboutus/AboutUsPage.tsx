...
export default function AboutUsPage() {
  ...
  const { data: itemData, isLoading } = useStaticPage('about_us')
  const saveMutation = useUpdateStaticPage('about_us')
  ...
  return (
    <Card className="border-border/60 shadow-sm rounded-2xl overflow-hidden bg-white">
      <CardHeader className="border-b border-gray-50 px-6 py-5">
        <CardTitle className="text-lg font-bold text-gray-800">{t('aboutUs') || 'About Us'}</CardTitle>
      </CardHeader>
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2 flex flex-col text-right" dir="rtl">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider block">
                  من نحن (عربي) <span className="text-rose-500">*</span>
                </label>
                <Controller
                  name="contentAr"
              ...
              <div className="space-y-2 flex flex-col text-left" dir="ltr">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider block">
                  About Us (English) <span className="text-rose-500">*</span>
                </label>
                <Controller
                  name="contentEn"
                  control={methods.control}
