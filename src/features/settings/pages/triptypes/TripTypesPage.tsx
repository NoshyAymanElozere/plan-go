...
export default function TripTypesPage() {
  ...
  const label = t('tripTypes') || 'TripTypes'
  const { data: items = [], isLoading } = useSettingsList('triptypes', search)

  const createMutation = useCreateSettingsItem('triptypes')
  const updateMutation = useUpdateSettingsItem('triptypes')
  const deleteMutation = useDeleteSettingsItem('triptypes')
