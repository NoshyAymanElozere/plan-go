...
import { CurrenciesFormModal } from './CurrenciesFormModal'

export default function CurrenciesPage() {
  const { t } = useTranslation()
  const [search, setSearch] = useState('')
  const label = t('currencies') || 'Currencies'
  const { data: items = [], isLoading } = useSettingsList('currencies', search)

  const createMutation = useCreateSettingsItem('currencies')
  const updateMutation = useUpdateSettingsItem('currencies')
  const deleteMutation = useDeleteSettingsItem('currencies')
